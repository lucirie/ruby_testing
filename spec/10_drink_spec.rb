...
describe Drink do
  describe '#initialize' do
    context 'when type is specified and ounces is default' do
      ...
      # beverage type.
      subject(:preferred_drink) { described_class.new(type='coffee', 16) }

      # remove the 'x' before running this test
      it 'is your choice of beverage' do
        expect(preferred_drink.type).to eq('coffee')
      end

      # remove the 'x' before running this test
      it 'has 16 ounces' do
        expect(preferred_drink).to be_full
      end
    end
  end
  ...
  describe '#full?' do
    context 'when drink has 16 ounces or more' do
      # Create an explicit subject, using 'described_class' and your choice of
      # beverage type.
      subject(:excessive_drink) { described_class.new(type='coffee', ounces=17) }

      # remove the 'x' before running this test
      it 'is full' do
        expect(excessive_drink).to be_full
      end
    end
    ...
    context 'when drink has less than 16 ounces' do
      ...
      subject(:minimal_drink) { described_class.new(type='coffee', ounces=7) }
      # remove the 'x' before running this test
      it 'is not full' do
        expect(minimal_drink).not_to be_full
      end
    end
  end
end
