...
describe FindNumber do
  ...
  describe '#game_over?' do
    context 'when guess and random_number are equal' do
      ...
      let(:random_number) { double('random_number', value: 3) }
      subject(:game_guessing) { described_class.new(4, 2, random_number) }
      ...
      it 'is game over' do
        expect(game_guessing.game_over?).to be_truthy
      end
    end
    ...
    context 'when guess and random_number are not equal' do
      ...
      it 'is not game over' do
        expect(game_guessing.game_over?).not_to be_truthy
      end
    end
  end
  ...
  describe '#update_range' do
    ...
    context 'when the guess is 7, min=5, and max=8' do
      ...
      subject(:edge_case_game) { described_class.new(min=5, max=8, random_number, guess=7) }
      it 'updates min to the same value as max' do
        edge_case_game.update_range
        new_min = edge_case_game.min
        expect(new_min).to eq(edge_case_game.max)
      end

      it 'does not update max' do
        old_max = edge_case_game.max
        edge_case_game.update_range
        new_max = edge_case_game.max

        expect(new_max).to eq(old_max)
      end
    end
  end
end
