...
describe BinaryGame do
  ...
  describe '#player_input' do
    ...
    context 'when user inputs an incorrect value once, then a valid input' do
      ...
      it 'completes loop and displays error message once' do
        min, max = game_input.instance_variable_get(:@minimum), game_input.instance_variable_get(:@maximum)
        input_error_string = "Input error! Please enter a number between #{min} or #{max}."
        expect(game_input).to receive(:puts).with(input_error_string).once
        game_input.player_input(min, max)
      end
    end
    ...
    context 'when user inputs two incorrect values, then a valid input' do
      before do
        incorrect_value1 = '89'
        incorrect_value2 = 'mona my love'
        valid_input = '5'

        allow(game_input).to receive(:gets).and_return(incorrect_value1, incorrect_value2, valid_input)
      end
  ...
  describe '#verify_input' do
    ...
    context 'when given a valid input as argument' do
      it 'returns valid input' do
        valid_input = 3
        expect(game.verify_input(0, 10, valid_input)).to be_truthy
      end
    end
    ...
    context 'when given invalid input as argument' do
      it 'returns nil' do
        invalid_input = 69
        expect(game.verify_input(0, 10, invalid_input)).to be_nil
      end
    end
  end
  ...
  describe '#display_turn_order' do
    ...
    let(:game) { BinaryGame.new(0, 10) }
    let(:binary_search_dbl) { instance_double(BinarySearch) }
    before do
      # You'll need to create a few method stubs.
      allow(binary_search_dbl).to receive(:make_guess)
      allow(binary_search_dbl).to receive(:update_range)
    ...
    it 'increases guess_count by one' do
      game.instance_variable_set(:@guess_count, 0)
      game.display_turn_order(binary_search_dbl)
      expect(game.instance_variable_get(:@guess_count)).to eq(1)
    end

    # Method with Outgoing Command -> Test that a message is sent
    it 'sends make_guess' do
      expect(binary_search_dbl).to receive(:make_guess).once
      game.display_turn_order(binary_search_dbl)
    end

    # Method with Outgoing Command -> Test that a message is sent
    it 'sends update_range' do
      expect(binary_search_dbl).to receive(:update_range).once
      game.display_turn_order(binary_search_dbl)
    end
